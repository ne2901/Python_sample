...
def register_user(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            email_body = "Hey %s, thanks for signing up @ rEc$corE. To activate your account, click this link within 48hours http://127.0.0.1:8000/accounts/confirm/%s" % (
...
def myprofile(request):
    ...
    return render_to_response('userprofile/myprofile.html',
                              {'testdata': testdata, 'user_profile': user_profile, 'mycomment': mycomment,
                               'query_results': query_results, 'user': user, 'form': form},
    ...
    def add_candidate(request):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect('/userprof      ile/workspace/')
            else:
                print form.errors
        else:

            form = CandidateForm()
        candidatee = Candidate.objects.filter(user=request.user).order_by('id')
        return render(request, 'userprofile/workspace.html', {'form': form, 'candidatee': candidatee},
                      context_instance=RequestContext(request))
